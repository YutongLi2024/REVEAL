...
from M3SRec.M3SRec import M3SRec
...
class M3SRecTrainer(Trainer):

    def __init__(self, model,
                 train_dataloader,
                 eval_dataloader,
                 test_dataloader, args):
        super(M3SRecTrainer, self).__init__(
            model,
            train_dataloader,
            eval_dataloader,
            test_dataloader, args
        ...
        # ===== AVL running statistics (paper-aligned) =====
        # bar(L_v), bar(L_t) are cumulative averages over steps
        self._avl_step = 0
        self._avl_bar_Lv = 0.0
        self._avl_bar_Lt = 0.0
        # ==================================================
    ...
    def bpr_optimization(self, seq_mean_out, seq_cov_out, pos_ids, neg_ids):
        ...
        seq_mean_emb = seq_mean_out.view(-1, self.args.hidden_size)
        seq_cov_emb = seq_cov_out.view(-1, self.args.hidden_size)
    ...
    # ====== AVL utils: grad norms (for logging only) ======
    def _grad_l2_norm(self, named_params, keywords):
        sq_sum = 0.0
        found = False
        for name, p in named_params:
    ...
    def _collect_modal_grad_norms(self):
        named_params = list(self.model.named_parameters())
        g_img = self._grad_l2_norm(named_params, ["image", "img", "vision", "task2", "dnn2", "fc_mean_image", "fc_cov_image"])
        g_txt = self._grad_l2_norm(named_params, ["text", "txt", "task3", "dnn3", "fc_mean_text", "fc_cov_text"])
        g_id  = self._grad_l2_norm(named_params, [
            "item_mean_embeddings", "item_cov_embeddings",
            "position_embeddings",
            "item_encoder", "encoder"
    ...
    # ====== AVL core: compute K_v (paper-aligned) ======
    @torch.no_grad()
    def _avl_compute_Kv(self, L_v: torch.Tensor, L_t: torch.Tensor):
        """
        Adaptive Visual Learning (AVL), paper:
          Δ_x^v = L_x^v - L_x^t
          r_x^v = exp(Δ_x^v)
          bar(L)_x^v, bar(L)_x^t : cumulative mean
          bar(r)_x^v = exp(bar(L)_x^v - bar(L)_x^t)
          K_x^v = exp( alpha * ( r_x^v - bar(r)_x^v ) )
        """
        alpha = float(getattr(self.args, "avl_alpha", 1.0))

        Lv = L_v.detach()
        Lt = L_t.detach()

        # update running averages of unimodal losses
        self._avl_step += 1
        t = self._avl_step
        self._avl_bar_Lv = self._avl_bar_Lv * (t - 1) / t + float(Lv) / t
        self._avl_bar_Lt = self._avl_bar_Lt * (t - 1) / t + float(Lt) / t

        # instantaneous modality gap
        delta_v = Lv - Lt                  # Δ_x^v
        r_v = torch.exp(delta_v)           # r_x^v

        # historical reference gap
        bar_r_v = np.exp(self._avl_bar_Lv - self._avl_bar_Lt)
        bar_r_v = torch.tensor(bar_r_v, device=r_v.device, dtype=r_v.dtype)  # \bar{r}_x^v

        # calibration factor
        K_v = torch.exp(alpha * (r_v - bar_r_v))  # K_x^v

        # optional clamp for stability
        k_min = float(getattr(self.args, "avl_kappa_min", 0.1))
        k_max = float(getattr(self.args, "avl_kappa_max", 10.0))
        K_v = torch.clamp(K_v, k_min, k_max)

        return K_v
    # ===================================================

    # ====== AVL: scale grads of visual-related params θ^v ======
    def _avl_scale_visual_grads(self, K_v: torch.Tensor):
        """
        Scale ONLY vision-related parameters θ^v (paper).
        For M3SRec backbone, vision-related params typically include:
          - image_mean_embeddings / image_cov_embeddings
          - fc_mean_image / fc_cov_image
          - experts_task2_* , dnn2_* (image-specific experts/gates)
        """
        for name, p in self.model.named_parameters():
            if p.grad is None:
                continue
            ...
            is_visual = (
                ("image" in n) or
                ("fc_mean_image" in n) or ("fc_cov_image" in n) or
                ("experts_task2" in n) or ("dnn2" in n) or
                ("task2" in n)   # 兜底：你有些实现可能是 task2_mean/task2_cov
            )

            # 防止误把 text 分支也算进来（极端情况下 name 同时含 image/text）
            is_text = (("text" in n) or ("task3" in n) or ("dnn3" in n))
    ...
    def iteration(self, epoch, dataloader, full_sort=False, train=True):
        ...
        if train:
            ...
            for batch in tqdm(rec_data_iter, desc="Training Progress"):
                ...
                # ===== 1) AVL: unimodal losses -> K_v (no grad) =====
                if getattr(self.args, "use_avl", True) and self.args.is_use_mm:
                    with torch.no_grad():
                        # Vision-only: keep image, mask text (modal="img")
                        seq_mean_v, seq_cov_v, _, _ = self.model.finetune(input_ids, user_ids, modal="img")
                        loss_v, _, pvn_v = self.bpr_optimization(seq_mean_v, seq_cov_v, target_pos, target_neg)
                        L_v = loss_v + pvn_v

                        # Text-only: keep text, mask image (modal="txt")
                        seq_mean_t, seq_cov_t, _, _ = self.model.finetune(input_ids, user_ids, modal="txt")
                        loss_t, _, pvn_t = self.bpr_optimization(seq_mean_t, seq_cov_t, target_pos, target_neg)
                        L_t = loss_t + pvn_t

                    K_v = self._avl_compute_Kv(L_v, L_t)
                    rec_avg_Kv += float(K_v.item())
                    rec_cnt_Kv += 1
                else:
                    K_v = None
                # ====================================================

                # ===== 2) full multimodal loss (normal training) =====
                seq_mean, seq_cov, _, _ = self.model.finetune(input_ids, user_ids, modal="full")
                loss, batch_auc, pvn_loss = self.bpr_optimization(seq_mean, seq_cov, target_pos, target_neg)
                loss = loss + pvn_loss

                self.optim.zero_grad()
                loss.backward()

                # ===== 3) AVL: scale ONLY visual grads =====
                if K_v is not None:
                    self._avl_scale_visual_grads(K_v)
                # ==========================================

                # grad norms logging (optional)
                g_img, g_txt, g_id = self._collect_modal_grad_norms()
                rec_avg_g_img += 0.0 if math.isnan(g_img) else g_img
                rec_avg_g_txt += 0.0 if math.isnan(g_txt) else g_txt
                rec_avg_g_id  += 0.0 if math.isnan(g_id)  else g_id
            ...
            if rec_cnt_Kv > 0:
                post_fix.update({
                    "K_v": "{:.4f}".format(rec_avg_Kv / rec_cnt_Kv),
                    "bar_Lv": "{:.4f}".format(self._avl_bar_Lv),
                    "bar_Lt": "{:.4f}".format(self._avl_bar_Lt),
                })
