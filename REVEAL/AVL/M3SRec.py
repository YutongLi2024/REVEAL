'''
We adopt M3SRec as the baseline backbone to demonstrate the plug-and-play integration of AVL and its training-time  process.
'''
...
class AVLMode(str, Enum):
    """
    full: use ID + IMG + TXT
    img : use ID + IMG, mask TXT
    txt : use ID + TXT, mask IMG
    id  : use only ID, mask IMG & TXT
    """
    FULL = "full"
    IMG  = "img"
    TXT  = "txt"
...
class AVLMaskCache:
    ...
    z_txt: Optional[torch.Tensor] = None
    sig_img: Optional[Tuple] = None
    sig_txt: Optional[Tuple] = None

    def _sig(self, x: torch.Tensor) -> Tuple:
        return (tuple(x.shape), x.device, x.dtype)

    def zeros_like_cached(self, x: torch.Tensor, which: str) -> torch.Tensor:
        sig = self._sig(x)
        if which == "img":
            if (self.z_img is None) or (self.sig_img != sig):
                self.z_img = torch.zeros_like(x)
                self.sig_img = sig
            return self.z_img
        elif which == "txt":
            if (self.z_txt is None) or (self.sig_txt != sig):
                self.z_txt = torch.zeros_like(x)
                self.sig_txt = sig
            return self.z_txt
        else:
            raise ValueError(f"Unknown cache key: {which}")
...
def avl_apply_mask(
    ...
    mode: str,
    cache: Optional[AVLMaskCache] = None,
) -> Tuple[torch.Tensor, torch.Tensor]:
    try:
        m = AVLMode(mode)
    except Exception:
        raise ValueError(f"Invalid AVL mode: {mode}. Use one of: {[e.value for e in AVLMode]}")

    if cache is None:
        z_img = torch.zeros_like(img_emb)
        z_txt = torch.zeros_like(txt_emb)
    else:
        z_img = cache.zeros_like_cached(img_emb, which="img")
        z_txt = cache.zeros_like_cached(txt_emb, which="txt")

    if m == AVLMode.FULL:
        return img_emb, txt_emb
    if m == AVLMode.IMG:
        return img_emb, z_txt
    if m == AVLMode.TXT:
        return z_img, txt_emb
    if m == AVLMode.ID:
        return z_img, z_txt
...
class M3SRec(nn.Module):
    ...
    def add_position_mean_embedding(self, sequence, modal="full"):
        ...
        # === AVL (optimized) ===
        item_image_embeddings, item_text_embeddings = avl_apply_mask(
            img_emb=item_image_embeddings,
            txt_emb=item_text_embeddings,
            mode=modal,
            cache=self._avl_cache,
        )
        # =======================
