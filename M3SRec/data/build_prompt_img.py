...
OUT_PROMPT_V2_PATH = os.path.join(DATASET_ROOT, "image_text", "Sports_promptV2_Neg_New_with_Brand.json")


PROMPT_OPT_INPUT_PATH = os.path.join("Sports_prompt_opt_input_all.json")


IMAGE_DIR = os.path.join(DATASET_ROOT, "image_text", "Sports_image")
...
def get_image_path_for_asin(asin: str) -> str | None:

    candidates = [
        os.path.join(IMAGE_DIR, f"{asin}.jpg"),
        os.path.join(IMAGE_DIR, f"{asin}.png"),
        os.path.join(IMAGE_DIR, f"{asin}.jpeg"),
    ]
    for p in candidates:
        if os.path.exists(p):
            return p
...
def qwen_generate_text(instr: str, asin: str | None = None,
                       max_new_tokens: int = MAX_NEW_TOKENS) -> str:

    image_path = None
    if asin is not None:
        image_path = get_image_path_for_asin(asin)
...
VERY IMPORTANT IDENTITY CONSTRAINTS:
- Treat Prompt V1 as the base identity of the item.
- You MUST preserve all correct identity cues from Prompt V1, including:
  brand name, product type, main usage category, and any key identifiers that define what the item is.
- Do NOT remove or change the brand or product type mentioned in Prompt V1.
- Prompt V2 should be a refinement of Prompt V1 that adds or re-weights visual cues, not a completely different caption.
...
2. NEW_PROMPT (Prompt V2):
   ...
   - Start from Prompt V1 and KEEP its brand name and product type unchanged.
     You are allowed to reorganize the sentence and add or remove secondary details,
     but you MUST still clearly mention the same brand and product type as in Prompt V1.
   ...
   - Prioritize user-valued cues over generic description.
     The result should NOT look like an ordinary generic product caption.
   - Remove details irrelevant to user preferences, but do not remove core identity (brand, category et.al).
...
NEW_PROMPT:
<Your improved English vision prompt (≤40 words), still containing the brand and product type from Prompt V1>
"""
    return instr
...
def optimize_single_prompt_with_feedback(sample, default_prompt_v1: str):
    ...
    try:
        assistant_text = qwen_generate_text(instr, asin=asin, max_new_tokens=MAX_NEW_TOKENS)
...
def main():
    ...
    for sample in tqdm(samples, desc="Optimizing prompts with feedback (Qwen local + image)"):
        asin = sample["asin"]
